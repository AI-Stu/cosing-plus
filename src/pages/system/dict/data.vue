      ...
      <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="字典标签" align="center" prop="dictLabel">
          <template #default="scope">
            <span
              v-if="(scope.row.tagType === '' || scope.row.tagType === 'primary') && (scope.row.tagClass === '' || scope.row.tagClass == null)"
            >{{ scope.row.dictLabel }}</span>
            <el-tag
              v-else
              :type="scope.row.tagType === 'processing' || scope.row.tagType === 'primary' ? 'processing' : scope.row.tagType"
              :class="scope.row.tagClass"
            >
              {{ scope.row.dictLabel }}
            </el-tag>
          </template>
...
// 数据标签回显样式
const tagTypeOptions = ref<Array<{ value: string, label: string }>>([
  { value: 'primary', label: '默认' },
  { value: 'processing', label: '主要' },
  { value: 'success', label: '成功' },
  { value: 'default', label: '信息' },
...
async function handleUpdate(row?: DictDataVO) {
  ...
  if (!dictCode)
    return proxy?.$modal.msgError('code 为空！');
